...
{
	const unsigned char AES_IV[16] = { 0x4D, 0x4E, 0x41, 0x40, 0x32, 0x30, 0x31, 0x37, 0x47, 0x4F, 0x48, 0x45, 0x41, 0x44, 0x21, 0x21 };
	const BYTE AES_KEY[16] = { 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x31, 0x32, 0x33, 0x34, 0x35, 0x35, 0x37, 0x38};

	int len = ct->len;
	int rest_len = len % AES_BLOCK_SIZE;
	int padding_len = ((ENCRYPT == mode) ? (AES_BLOCK_SIZE - rest_len) : 0);
	int src_len = len + padding_len;
	
	//input
	unsigned char *input = (unsigned char *)malloc(src_len);
	memset(input, 0, src_len);
	memcpy(input, ct->con, len);
	if (padding_len > 0) {
		memset(input + len, (char) padding_len, padding_len);
	}
	
	//output
	unsigned char *output = (unsigned char*) malloc(src_len);
	if (!output) {
		free(input);
	}
	...
	//exec encrypt/decrypt(CBC mode)
	if (mode == ENCRYPT) {
		aes_encrypt_cbc(input, src_len, output, key_schedule, AES_KEY_SIZE,
				AES_IV);
	} else {
		aes_decrypt_cbc(input, src_len, output, key_schedule, AES_KEY_SIZE,
				AES_IV);
	}


	//calc lenth of decrypt
	if (ENCRYPT != mode) {
...
/***********************************************************
*name		:	tq_crypto_aes
*function	:	tencent provide for crypto data through net socket
				use algorithm of aes
				use key offered itself
*argument	:	unsigned char *data	:	data to be encrypt/decrypt
				uint16_t len			:	lenth of data
				int flag				:	ENCRYPT/DECRYPT
*return		:	void
